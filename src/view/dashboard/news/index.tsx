...
const News: FC = () => {
  ...
  const getEntities = async (page = 1, sizePerPage = 10) => {
    ...
    try {
      const { data }: any = await api({
        url: `/news/admin/list/?page=${page}&size=${sizePerPage}${isQuery}${isType}`,
        method: "get",
      });
      if (data.serverResponse.isError) {
        ...
        setTotalData(data.result.data.countNews);
  ...
  const getSelectRow = {
    ...
    onSelect: (data: any) => history.push(`/news/update/${data._id}`),
  };
  ...
  return (
    <Card>
      <CardHeader title="News">
        <CardHeaderToolbar>
          <Link className="btn btn-primary" to="/news/add">
            Add
          </Link>
        </CardHeaderToolbar>
      </CardHeader>
