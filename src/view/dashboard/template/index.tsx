...
const Template: FC = () => {
  ...
  const getEntities = async (page = 1, sizePerPage = 10) => {
    ...
    try {
      const { data }: any = await api({
        url: `/template/list/?page=${page}&size=${sizePerPage}${isQuery}${isType}`,
        method: "get",
      });
      if (data.serverResponse.isError) {
        toast.error(data.serverResponse.message);
      } else {
        const entities = data.result.data.templateData.map(
        ...
        setTotalData(data.result.data.countTemplate);
  ...
  const getSelectRow = {
    ...
    onSelect: (data: any) => history.push(`/template/update/${data._id}`),
  };
  ...
  return (
    <Card>
      <CardHeader title="Template">
        <CardHeaderToolbar>
          <Link className="btn btn-primary" to="/template/add">
            Add
          </Link>
        </CardHeaderToolbar>
      </CardHeader>
