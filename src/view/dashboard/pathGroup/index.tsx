...
const PathGroup: FC = () => {
  ...
  const getEntities = async (page = 1, sizePerPage = 10) => {
    ...
    try {
      const { data }: any = await api({
        url: `/pathGroup/list/?page=${page}&size=${sizePerPage}${isQuery}${isType}`,
        method: "get",
      });
      if (data.serverResponse.isError) {
        toast.error(data.serverResponse.message);
      } else {
        const entities = data.result.data.pathGroupData.map(
          (entity: any, ind: number) => {
            return {
              ...entity,
              id: ++ind + (page - 1) * sizePerPage,
              status: entity.isActive ? "ACTIVE" : "INACTIVE",
              siteId: entity.siteId?.name ? entity.siteId?.name : "-",
            };
          }
        );
        setTotalData(data.result.data.countPathGroup);
  ...
  const getSelectRow = {
    ...
    onSelect: (data: any) => history.push(`/PathGroup/update/${data._id}`),
  };
  return (
    <Card>
      <CardHeader title="Path Group">
        <CardHeaderToolbar>
          <Link className="btn btn-primary" to="/PathGroup/add">
            Add
          </Link>
        </CardHeaderToolbar>
      </CardHeader>
