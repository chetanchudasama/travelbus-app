...
const Location: FC = () => {
  ...
  const getEntities = async (page = 1, sizePerPage = 10) => {
    ...
    try {
      const { data }: any = await api({
        url: `/location/list/?page=${page}&size=${sizePerPage}${isQuery}${isType}`,
        method: "get",
      });
      if (data.serverResponse.isError) {
        ...
        setTotalData(data.result.data.countLocation);
  ...
  const getSelectRow = {
    ...
    onSelect: (data: any) => history.push(`/location/update/${data._id}`),
  };
